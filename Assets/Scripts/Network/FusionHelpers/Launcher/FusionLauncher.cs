...
namespace FusionHelpers
{
	...
	public class FusionLauncher : SimulationBehaviour, INetworkRunnerCallbacks
	{
		...
		public static FusionLauncher Launch(GameMode mode, string room, FusionAppSettings appSettings, FusionSession sessionPrefab,
			INetworkSceneManager sceneLoader, int selectedLevel,
			Action<NetworkRunner, ConnectionStatus, string> onConnect)
		{
			...
			launcher.InternalLaunch(mode,room, appSettings, sessionPrefab, sceneLoader, selectedLevel, onConnect);
      return launcher;
    }
		
		private async void InternalLaunch(GameMode mode, string room, FusionAppSettings appSettings,
            FusionSession sessionPrefab,
			INetworkSceneManager sceneManager, int selectedLevel,
            Action<NetworkRunner, ConnectionStatus, string> onConnect)
		{
            ...
            customProps["map"] = (int)selectedLevel;

            NetworkSceneInfo scene = new NetworkSceneInfo();
				scene.AddSceneRef(SceneRef.FromIndex(selectedLevel));

			SetConnectionStatus(_runner, ConnectionStatus.Connecting, "");
			...
			await _runner.StartGame(new StartGameArgs()
			{
				...
				PlayerCount = FusionSession.MAX_PLAYERS,
